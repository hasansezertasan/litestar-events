...
- Should we use a managed queue (single queue for all events) or separate queues for each event type (queue_name = event_ids) on RabbitMQ and it's like? Which one is expected for Litestar Event System?
- Other Backends:
