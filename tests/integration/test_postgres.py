...
async def test_emit_delivers_to_listener(postgres_dsn):
...
@pytest.mark.integration
async def test_failing_listener_does_not_block_sibling(postgres_dsn):
...
@pytest.mark.integration
async def test_invalid_event_id_raises_at_enter(postgres_dsn):
